...
const CRMScreen: React.FC<CRMScreenProps> = ({ setView }) => {
    ...
    const renderActiveView = () => {
        switch(crmView.type) {
            case 'customer_form':
                return <CustomerFormPage customerId={crmView.customerId} onBack={() => setCrmView({ type: 'module', id: 'customers' })} />;
            case 'segment_form':
            ...
            default:
                switch(crmView.id) {
                    case 'customers':
                        return <CustomerManagement onNewCustomer={() => setCrmView({ type: 'customer_form' })} onEditCustomer={(id) => setCrmView({ type: 'customer_form', customerId: id })} />;
                    case 'segmentation':
    ...
    const getHeaderInfo = () => {
        switch (crmView.type) {
            case 'customer_form': return { title: crmView.customerId ? 'Editar Cliente' : 'Novo Cliente', activeModule: 'customers' as ActiveCrmModule, backView: { type: 'module', id: 'customers' } as CrmView };
        ...
        <div className="flex flex-col md:flex-row h-full bg-gray-100 dark:bg-gray-900">
            ...
            <main className="flex-1 flex flex-col overflow-hidden order-2 h-full">
                <header className="bg-white dark:bg-gray-800 shadow-sm p-4 flex items-center gap-4 shrink-0">
                    {backView && <Button variant="secondary" onClick={() => setCrmView(backView)} className="!py-2 !px-3"><ArrowLeft size={16} className="mr-2"/> Voltar</Button>}

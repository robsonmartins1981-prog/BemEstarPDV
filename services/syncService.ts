...
async function processFiscalEmission(payload: { saleId: string }): Promise<boolean> {
    try {
        ...
        if (!sale) {
            ...
            return false; // Retorna false mas talvez devesse ser true para remover da fila se a venda não existe? 
                          // Neste caso, vamos considerar erro para retry ou análise manual.
        }
